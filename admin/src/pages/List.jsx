  ...
  const fetchList = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        if (!errorToastId.current) {
          errorToastId.current = toast.error(response.data.message);
        }
      }
      ...
      if (error.response) {
        ...
        if (!errorToastId.current) {
          errorToastId.current = toast.error(error.response.data.message || error.message);
        }
      } else {
        console.log(error)
        if (!errorToastId.current) {
          errorToastId.current = toast.error(error.message);
        }
      }
    }
  }
